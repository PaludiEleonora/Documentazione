...
**code doc.txt** ==> Apre il documento con visual studio
...
## Merge ( PROVA MIA, SOTTO PIU' DETTAGLIATO)
...
-	Risolvi i conflitti:
-	Apri i file contrassegnati come in conflitto.
-	Cerca i marcatori di conflitto (<<<<<<<, =======, >>>>>>>) e risolvi manualmente.
-	Completa il merge: Dopo aver risolto i conflitti, aggiungi i file risolti e crea il commit:
git add file.txt
git commit
...
-	Identifica i file in conflitto Dopo un merge interrotto, esegui:
git status

-	Risolvi i conflitti manualmente
	Apri i file indicati.
	Cerca i marcatori di conflitto, ad esempio: sviluppo
	Scegli quale versione mantenere, oppure combina le modifiche, eliminando i marcatori.
    Aggiungi i file risolti Una volta risolti i conflitti:
    **git add file.txt**
	Completa il merge Dopo aver aggiunto tutti i file risolti:
    git commit
...
- Controllare un merge prima di eseguirlo
...
**git checkout --theirs nomefile**
	--ours: Mantiene la versione del ramo attuale.
	--theirs: Mantiene la versione del ramo unito.
